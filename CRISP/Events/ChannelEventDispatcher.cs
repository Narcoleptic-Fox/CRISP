...
public class ChannelEventDispatcher : IEventDispatcher, IDisposable, IAsyncDisposable
{
        ...
        ChannelEventOptions channelOptions)
    {
        ...
        _channel = _channelOptions.ChannelCapacity > 0
            ? Channel.CreateBounded<IEvent>(new BoundedChannelOptions(_channelOptions.ChannelCapacity)
            {
    ...
    private async Task ConsumeEventsAsync(int consumerId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                while (_channel.Reader.TryRead(out IEvent? @event))
                {
                    try
                    {
    ...
    /// </summary>
    private async Task InvokeHandlerAsync(object handler, IEvent @event, Type eventType, CancellationToken cancellationToken)
    {
        try
        {
    ...
    public async ValueTask Dispatch(IEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        if (_eventOptions.EnableDetailedLogging)
            _logger.LogDebug("Enqueuing domain event {EventType}", @event.GetType().Name);
    ...
    public async ValueTask DispatchAll(IEnumerable<IEvent> events, CancellationToken cancellationToken = default)
    {
        ...
        if (eventsList.Count > _eventOptions.MaxBatchSize)
        {
            ...
            {
                IEnumerable<IEvent> batch = eventsList.Skip(i).Take(_eventOptions.MaxBatchSize);
    ...
    /// </summary>
    private async ValueTask DispatchBatchAsync(IEnumerable<IEvent> events, CancellationToken cancellationToken)
    {
